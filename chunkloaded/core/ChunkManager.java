...
public class ChunkManager {
   ...
   */
  public Set<class_1923> calculateGlobalFrontierChunks() {
    Set<class_1923> allUnlockedChunks = getGlobalUnlockedChunks();
    Set<class_1923> frontier = new HashSet<>();

    // For each unlocked chunk (any player), check all 4 cardinal neighbors
    for (class_1923 unlocked : allUnlockedChunks) {
      class_1923[] neighbors = {
          new class_1923(unlocked.field_9181, unlocked.field_9180 - 1), // North
          new class_1923(unlocked.field_9181, unlocked.field_9180 + 1), // South
          new class_1923(unlocked.field_9181 + 1, unlocked.field_9180), // East
          new class_1923(unlocked.field_9181 - 1, unlocked.field_9180) // West
      };

      for (class_1923 neighbor : neighbors) {
   ...
   */
  @Deprecated
  public boolean isChunkUnlocked(UUID player, class_1923 pos) {
   ...
   * @return true if the chunk is unlocked, false otherwise
   */
  public boolean isChunkUnlockedGlobally(class_1923 pos) {
    return chunkData.isChunkUnlocked(pos);
  }
   ...
   */
  @Deprecated
  public Set<class_1923> getPlayerChunks(UUID player) {
    return chunkData.getGlobalUnlockedChunks();
  }
  ...
  public List<PlayableArea> getPlayerAreas(UUID player) {
    Set<class_1923> unlockedChunks = chunkData.getGlobalUnlockedChunks();
   ...
   */
  @Deprecated
  public Optional<PlayableArea> getAreaContainingChunk(UUID player, class_1923 chunk) {
    Set<class_1923> unlockedChunks = chunkData.getGlobalUnlockedChunks();
   ...
   * @return An Optional containing the area if found
   */
  public Optional<PlayableArea> getGlobalAreaContainingChunk(class_1923 chunk) {
    Set<class_1923> unlockedChunks = chunkData.getGlobalUnlockedChunks();
    List<PlayableArea> areas = areaDetector.detectAreas(unlockedChunks);
    return areas.stream()
        .filter(area -> area.contains(chunk))
   ...
   */
  public boolean tryUnlockChunk(UUID player, class_1923 pos) {
   ...
   */
  public int tryUnlockChunks(UUID player, Set<class_1923> positions) {
    int unlockedCount = 0;
   ...
   * @param pos    The chunk position to unlock
   */
  public void forceUnlockChunk(UUID player, class_1923 pos) {
   ...
   */
  public void forceUnlockChunk(UUID player, class_1923 pos, class_3218 world) {
    if (!isChunkUnlockedGlobally(pos)) {
      chunkData.unlockChunk(pos);
      chunkData.markDirty();
      areaDetector.invalidateCache();
   ...
   */
  public int forceUnlockChunks(UUID player, Set<class_1923> positions) {
    int unlockedCount = 0;
    for (class_1923 pos : positions) {
      if (!isChunkUnlockedGlobally(pos)) {
        chunkData.unlockChunk(pos);
   ...
   */
  public void lockChunk(UUID player, class_1923 pos) {
    if (isChunkUnlockedGlobally(pos)) {
   ...
   */
  public int lockChunks(UUID player, Set<class_1923> positions) {
    int lockedCount = 0;
    for (class_1923 pos : positions) {
      if (isChunkUnlockedGlobally(pos)) {
        chunkData.lockChunk(pos);
   ...
   */
  public void updateBarriersAfterUnlock(class_3218 world, UUID player, class_1923 unlockedChunk) {
    updateBarriersForUnlock(world, player, unlockedChunk);
  }

  private void updateBarriersForUnlock(class_3218 world, UUID player, class_1923 unlockedChunk) {
    ...
    Set<class_1923> allUnlocked = getGlobalUnlockedChunks();
    ...
    // chunk)
    Set<class_1923> allLocked = calculateGlobalFrontierChunks();
   ...
   * @param player The player UUID
   */
  public void initializeBarriersForPlayer(class_3218 world, UUID player) {
    ...
    Set<class_1923> globalUnlockedChunks = getGlobalUnlockedChunks();
    LOGGER.info("Global state: {} total unlocked chunks across all players", globalUnlockedChunks.size());

    Set<class_1923> globalFrontierChunks = calculateGlobalFrontierChunks();
    ...
    for (class_1923 lockedChunk : globalFrontierChunks) {
      LOGGER.debug("Placing barriers around frontier chunk [{}, {}]", lockedChunk.field_9181, lockedChunk.field_9180);
   ...
   */
  public Set<class_1923> calculateFrontierChunks(UUID player) {
    Set<class_1923> unlockedChunks = getPlayerChunks(player);
    Set<class_1923> frontier = new HashSet<>();

    // For each unlocked chunk, check all 4 cardinal neighbors
    for (class_1923 unlocked : unlockedChunks) {
      class_1923[] neighbors = {
          new class_1923(unlocked.field_9181, unlocked.field_9180 - 1), // North
          new class_1923(unlocked.field_9181, unlocked.field_9180 + 1), // South
          new class_1923(unlocked.field_9181 + 1, unlocked.field_9180), // East
