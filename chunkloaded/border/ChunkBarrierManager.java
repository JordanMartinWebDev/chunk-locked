...
public class ChunkBarrierManager {
  ...
  // World height constants for 1.21
   ...
   */
  private final Map<class_1923, Set<class_2338>> chunkBarriers = new ConcurrentHashMap<>();
   ...
   */
  private final Set<class_2338> allBarrierPositions = ConcurrentHashMap.newKeySet();
   ...
   */
  public void placeBarriers(class_3218 world, class_1923 lockedChunk, Set<class_1923> unlockedChunks) {
    LOGGER.info("placeBarriers called for chunk [{}, {}]", lockedChunk.field_9181, lockedChunk.field_9180);
    if (world == null || lockedChunk == null || unlockedChunks == null) {
      LOGGER.warn("Cannot place barriers: null parameter provided");
      return;
    }

    Set<class_2338> barriers = new HashSet<>();

    // Check all 4 cardinal directions
    class_1923 north = new class_1923(lockedChunk.field_9181, lockedChunk.field_9180 - 1);
    class_1923 south = new class_1923(lockedChunk.field_9181, lockedChunk.field_9180 + 1);
    class_1923 east = new class_1923(lockedChunk.field_9181 + 1, lockedChunk.field_9180);
    class_1923 west = new class_1923(lockedChunk.field_9181 - 1, lockedChunk.field_9180);
    ...
    if (!barriers.isEmpty()) {
      ...
      LOGGER.debug("Placed {} barriers around chunk [{}, {}]",
          barriers.size(), lockedChunk.field_9181, lockedChunk.field_9180);
    }
  }
   ...
   * @param world The server world
   */
  public void clearAllBarriers(class_3218 world) {
    if (world == null) {
    ...
    for (Map.Entry<class_1923, Set<class_2338>> entry : chunkBarriers.entrySet()) {
   ...
   */
  public void removeBarriers(class_3218 world, class_1923 chunk) {
    ...
    Set<class_2338> trackedBarriers = chunkBarriers.remove(chunk);
    int removedFromTracking = 0;
    if (trackedBarriers != null && !trackedBarriers.isEmpty()) {
      for (class_2338 pos : trackedBarriers) {
        world.method_8501(pos, class_2246.field_10124.method_9564());
    ...
    LOGGER.debug("Removed {} tracked + {} stray barriers from chunk [{}, {}]",
        removedFromTracking, removedFromScan, chunk.field_9181, chunk.field_9180);
  }
   ...
   */
  private int removeStrayBarriers(class_3218 world, class_1923 chunk) {
    int removed = 0;
    int chunkWorldX = chunk.field_9181 * CHUNK_SIZE;
    int chunkWorldZ = chunk.field_9180 * CHUNK_SIZE;
    ...
    for (int x = chunkWorldX; x < chunkWorldX + CHUNK_SIZE; x++) {
      for (int y = MIN_WORLD_Y; y <= MAX_WORLD_Y; y++) {
        class_2338 pos = new class_2338(x, y, chunkWorldZ);
        if (world.method_8320(pos).method_27852(Chunklocked.BARRIER_BLOCK_V2)) {
          world.method_8501(pos, class_2246.field_10124.method_9564());
          allBarrierPositions.remove(pos);
          removed++;
        }
    ...
    for (int x = chunkWorldX; x < chunkWorldX + CHUNK_SIZE; x++) {
      for (int y = MIN_WORLD_Y; y <= MAX_WORLD_Y; y++) {
        class_2338 pos = new class_2338(x, y, chunkWorldZ + CHUNK_SIZE - 1);
        if (world.method_8320(pos).method_27852(Chunklocked.BARRIER_BLOCK_V2)) {
          world.method_8501(pos, class_2246.field_10124.method_9564());
          allBarrierPositions.remove(pos);
          removed++;
    ...
    for (int z = chunkWorldZ; z < chunkWorldZ + CHUNK_SIZE; z++) {
      for (int y = MIN_WORLD_Y; y <= MAX_WORLD_Y; y++) {
        class_2338 pos = new class_2338(chunkWorldX, y, z);
        if (world.method_8320(pos).method_27852(Chunklocked.BARRIER_BLOCK_V2)) {
          world.method_8501(pos, class_2246.field_10124.method_9564());
          allBarrierPositions.remove(pos);
          removed++;
    ...
    for (int z = chunkWorldZ; z < chunkWorldZ + CHUNK_SIZE; z++) {
      for (int y = MIN_WORLD_Y; y <= MAX_WORLD_Y; y++) {
        class_2338 pos = new class_2338(chunkWorldX + CHUNK_SIZE - 1, y, z);
        if (world.method_8320(pos).method_27852(Chunklocked.BARRIER_BLOCK_V2)) {
          world.method_8501(pos, class_2246.field_10124.method_9564());
          allBarrierPositions.remove(pos);
          removed++;
   ...
   */
  public int removeStrayBarriersAtBoundary(class_3218 world, class_1923 chunk1, class_1923 chunk2) {
    ...
    if (chunk2.field_9181 == chunk1.field_9181 && chunk2.field_9180 == chunk1.field_9180 - 1) {
      // chunk2 is north of chunk1
      int x1World = chunk1.field_9181 * CHUNK_SIZE;
      int z1World = chunk1.field_9180 * CHUNK_SIZE;
      for (int x = x1World; x < x1World + CHUNK_SIZE; x++) {
        for (int y = MIN_WORLD_Y; y <= MAX_WORLD_Y; y++) {
          class_2338 pos = new class_2338(x, y, z1World);
          if (world.method_8320(pos).method_27852(Chunklocked.BARRIER_BLOCK_V2)) {
            world.method_8501(pos, class_2246.field_10124.method_9564());
            allBarrierPositions.remove(pos);
            removed++;
    ...
    } else if (chunk2.field_9181 == chunk1.field_9181 && chunk2.field_9180 == chunk1.field_9180 + 1) {
      // chunk2 is south of chunk1
      int x1World = chunk1.field_9181 * CHUNK_SIZE;
      int z1World = chunk1.field_9180 * CHUNK_SIZE;
      for (int x = x1World; x < x1World + CHUNK_SIZE; x++) {
        for (int y = MIN_WORLD_Y; y <= MAX_WORLD_Y; y++) {
          class_2338 pos = new class_2338(x, y, z1World + CHUNK_SIZE - 1);
          if (world.method_8320(pos).method_27852(Chunklocked.BARRIER_BLOCK_V2)) {
            world.method_8501(pos, class_2246.field_10124.method_9564());
            allBarrierPositions.remove(pos);
            removed++;
    ...
    } else if (chunk2.field_9181 == chunk1.field_9181 - 1 && chunk2.field_9180 == chunk1.field_9180) {
      // chunk2 is west of chunk1
      int x1World = chunk1.field_9181 * CHUNK_SIZE;
      int z1World = chunk1.field_9180 * CHUNK_SIZE;
      for (int z = z1World; z < z1World + CHUNK_SIZE; z++) {
        for (int y = MIN_WORLD_Y; y <= MAX_WORLD_Y; y++) {
          class_2338 pos = new class_2338(x1World, y, z);
          if (world.method_8320(pos).method_27852(Chunklocked.BARRIER_BLOCK_V2)) {
            world.method_8501(pos, class_2246.field_10124.method_9564());
            allBarrierPositions.remove(pos);
            removed++;
      ...
      for (int z = z1World; z < z1World + CHUNK_SIZE; z++) {
        for (int y = MIN_WORLD_Y; y <= MAX_WORLD_Y; y++) {
          class_2338 pos = new class_2338(x1World + CHUNK_SIZE - 1, y, z);
          if (world.method_8320(pos).method_27852(Chunklocked.BARRIER_BLOCK_V2)) {
            world.method_8501(pos, class_2246.field_10124.method_9564());
            allBarrierPositions.remove(pos);
            removed++;
   ...
   *                          unlocked)
   */
  public void updateBarriers(class_3218 world, class_1923 unlockedChunk,
      Set<class_1923> allUnlockedChunks, Set<class_1923> allLockedChunks) {
    ...
    class_1923[] adjacentChunks = {
        new class_1923(unlockedChunk.field_9181, unlockedChunk.field_9180 - 1), // North
        new class_1923(unlockedChunk.field_9181, unlockedChunk.field_9180 + 1), // South
        new class_1923(unlockedChunk.field_9181 + 1, unlockedChunk.field_9180), // East
        new class_1923(unlockedChunk.field_9181 - 1, unlockedChunk.field_9180) // West
    };

    for (class_1923 adjacent : adjacentChunks) {
    ...
    LOGGER.debug("Updated barriers after unlocking chunk [{}, {}]",
        unlockedChunk.field_9181, unlockedChunk.field_9180);
  }
   ...
   */
  public boolean shouldHaveBarrier(class_3218 world, class_2338 pos, java.util.UUID playerUuid, boolean wasBarrier) {
    ...
    int localX = pos.method_10263() & 15; // Position within chunk (0-15)
    ...
    if (atNorthEdge) {
      class_1923 north = new class_1923(currentChunk.field_9181, currentChunk.field_9180 - 1);
    ...
    if (atSouthEdge) {
      class_1923 south = new class_1923(currentChunk.field_9181, currentChunk.field_9180 + 1);
    ...
    if (atWestEdge) {
      class_1923 west = new class_1923(currentChunk.field_9181 - 1, currentChunk.field_9180);
    ...
    if (atEastEdge) {
      class_1923 east = new class_1923(currentChunk.field_9181 + 1, currentChunk.field_9180);
   ...
   */
  private Set<class_2338> createBarrierWall(class_3218 world, class_1923 chunk, Direction direction) {
    Set<class_2338> barriers = new HashSet<>();
    ...
    for (int y = MIN_WORLD_Y; y <= MAX_WORLD_Y; y++) {
      for (int parallel = parallelStart; parallel < parallelEnd; parallel++) {
        ...
        if (isXAxis) {
          ...
          pos = new class_2338(parallel, y, wallZ);
        } else {
          // East or West wall (vary Z)
          pos = new class_2338(wallX, y, parallel);
        }
        ...
        if (existingState.method_26215() || existingState.method_45474()) {
          world.method_8501(pos, Chunklocked.BARRIER_BLOCK_V2.method_9564());
          barriers.add(pos);
        }
